...
dag_config = Variable.get('t_agg_dly_subscriber_usage_dag_variables', deserialize_json=True, default_var={})
t_agg_dly_load_sql_statement = Variable.get('t_agg_dly_load_sql', default_var={})
t_agg_dly_timestamp_sql_statement = Variable.get('t_agg_dly_timestamp_sql', default_var={})
...
# DAG object.
with models.DAG(
        dag_id='t_agg_dly_subscriber_usage_dag',
    ...
    list_files = bash.BashOperator(
        task_id="list_files",
        bash_command=f"gsutil du gs://bt-dev-raw-ccn-mob-nwk-edwh-agg-dly-subscriber-usage/EE_T_AGG_DLY*.gz | wc -l",
        do_xcom_push=True,
    )

    save_file_count = PythonOperator(
        task_id='save_file_count',
        python_callable=file_count
    )
  ...
  # added record_ts update in insert/ load statement  
...
#     )  
    ...
    list_files >> save_file_count >> start_gcs_to_bq_job >> load_job >> backup_raw_file 
