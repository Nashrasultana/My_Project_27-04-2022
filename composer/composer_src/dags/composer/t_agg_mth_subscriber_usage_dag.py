...
dag_config = Variable.get('t_agg_mth_subscriber_usage_dag_variables', deserialize_json=True, default_var={})
t_agg_mth_load_sql_statement = Variable.get('t_agg_mth_load_sql', default_var={})
t_agg_mth_timestamp_sql_statement = Variable.get('t_agg_mth_timestamp_sql', default_var={})
...
agg_mth_destination_dataset = dag_config.get('agg_mth_destination_dataset')
...
table_id="bt-tch-3g-sunset-dp-dev.net_mobnet_rw.t_agg_mth_subscriber_usage"

GCP_DEFAULT_CONN = 'google_cloud_default'
next_month=""
latest_month=""
current_month=""
initial_record_cnt=0


def get_system_dates(**kwargs):
    ...
    current_month = today[:4] + today[5:7]
    ...
    print("\n This execution will load data for month ",current_month)
    

def get_table_count(**kwargs):
    from google.cloud import logging
    from google.cloud import bigquery
    ...
    client.setup_logging()
    bq_client = bigquery.Client()
    global  final_record_cnt

    table=bq_client.get_table(table_id)

    final_record_cnt=table.num_rows

    print("After sucessful execution record count in table:",final_record_cnt)
...
# DAG object.
with models.DAG(
        dag_id='t_agg_mth_subscriber_usage_dag',
    ...
    #     bucket='bt-dev-raw-ccn-net-mobnet-edwh-agg-mth-subs-usage',
    ...
    #     task_id="dataflow_operator_t_agg_mth_subscriber_usage_batch",
    ...
    print_dates = PythonOperator(
        task_id='get_system_dates',
        python_callable=get_system_dates,
        do_xcom_push=True,
    )
    command="gsutil cat gs://bt-dev-stg-ccn-mob-nwk-edwh/monthly_subscriber_usage/stage_tkn/response/EE_T_AGG_MTH*"+current_month+"_006_016_HASH.dat | tail -1"
    get_record_to_be_loaded = bash.BashOperator(
        task_id="records_loaded_count",
    ...
    load_job = BigQueryExecuteQueryOperator(
        ...
        sql=t_agg_mth_load_sql_statement,
        write_disposition="WRITE_APPEND",
        gcp_conn_id=GCP_DEFAULT_CONN,
        use_legacy_sql=False,
    )
    
    get_final_rec_cnt = PythonOperator(
        task_id='get_final_rec_cnt',
    ...
    insert_timestamp = BigQueryExecuteQueryOperator(
        ...
        sql=t_agg_mth_timestamp_sql_statement,
        write_disposition="WRITE_APPEND",
        gcp_conn_id=GCP_DEFAULT_CONN,
       use_legacy_sql=False,
    )
...
#     )
   ...
   ##     source_bucket=raw_bucket_name,
    #    source_objects=[input_file,hash_file],
    #    destination_bucket=raw_bucket_name,
    #    destination_object='backup' + "/" + 'BKP_EE_T_AGG_MTH',
    #    move_object=True
...
#     )  
    ...
    print_dates >> get_record_to_be_loaded >> start_gcs_to_bq_job >> load_job >> get_final_rec_cnt >> insert_timestamp
