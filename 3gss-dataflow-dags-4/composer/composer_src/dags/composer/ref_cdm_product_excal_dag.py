...
dag_config = Variable.get('ref_cdm_product_excal_dag_variables', deserialize_json=True, default_var={})
product_load_sql_statement = Variable.get('product_load_sql', default_var={})
product_timestamp_sql_statement = Variable.get('product_timestamp_sql', default_var={})
...
# DAG object.
with models.DAG(
        dag_id='ref_cdm_product_excal_dag',
    ...
    Clear_table = BigQueryExecuteQueryOperator(
        task_id="Clear_table",
        sql=""" delete from `bt-tch-3g-sunset-dp-prod.dp_net_mobnet_rw.ref_cdm_product_excal` where 1=1 """,
    ...
    #     sql=""" insert into `bt-tch-3g-sunset-dp-prod.dp_net_mobnet_rw.ref_cdm_product_excal`(product_code,effective_date,expiration_date,protocol,product_rollup,base_type,product_group,product_description,total_allowance_data,family_type,base_type_level1,product_type,stack,brand) select product_code,effective_date,expiration_date,protocol,product_rollup,base_type,product_group,product_description,total_allowance_data,family_type,base_type_level1,product_type,stack,brand from `bt-con-fincdmi-dp-prod.con_dp_cdm_ref_ro.ref_cdm_product_excal` """,
    ...
    BQtoBQ_load_job = BigQueryExecuteQueryOperator(
        task_id="BQtoBQ_load_job",
        sql=product_load_sql_statement,
        write_disposition="WRITE_TRUNCATE",
        gcp_conn_id=GCP_DEFAULT_CONN,
        use_legacy_sql=False,
    )
    ...
    #     sql=""" update `bt-tch-3g-sunset-dp-prod.dp_net_mobnet_rw.ref_cdm_product_excal` set record_ts = current_timestamp() where record_ts IS NULL """,
    #     write_disposition="WRITE_APPEND",
    #     gcp_conn_id=GCP_DEFAULT_CONN,
    #     use_legacy_sql=False,
    # )
    ...
    insert_timestamp = BigQueryExecuteQueryOperator(
        task_id="insert_timestamp",
        sql=product_timestamp_sql_statement,
        write_disposition="WRITE_APPEND",
        gcp_conn_id=GCP_DEFAULT_CONN,
        use_legacy_sql=False,
    )
